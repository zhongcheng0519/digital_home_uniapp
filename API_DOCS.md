...
**请求示例**:
```
GET /api/v1/note/?family_id=1&category=地址信息
```
...
**说明**: 
- 结果按创建时间降序排列（最新的在前）
- 不传 `category` 参数则返回该家庭的所有便利贴

---

### 3. 更新便利贴

**接口**: `PUT /api/v1/note/{note_id}`
...
1. 用家庭密钥加密便利贴标题，得到 `title_ciphertext`
2. 用家庭密钥加密便利贴内容，得到 `content_ciphertext`
3. 发送创建便利贴请求

### 更新便利贴流程

1. 用家庭密钥加密新的标题（如果更新），得到 `title_ciphertext`
2. 用家庭密钥加密新的内容（如果更新），得到 `content_ciphertext`
3. 发送更新便利贴请求
